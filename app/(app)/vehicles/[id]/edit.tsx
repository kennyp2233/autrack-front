...
export default function EditVehicleScreen() {
    ...
    const { getVehicle, updateVehicle, refreshData } = useVehicles();
    ...
    const [vehicle, setVehicle] = useState(getVehicle(vehicleId));

    // Obtener datos iniciales
    useEffect(() => {
        if (vehicle) {
            setInitialValues({
                brand: vehicle.id_marca,
                model: vehicle.id_modelo,
                year: vehicle.anio.toString(),
                mileage: vehicle.kilometraje_actual.toString(),
                plate: vehicle.placa || ''
            });
    ...
    const handleSubmit = async (values: FormValues) => {
        try {
            ...
            const vehicleData: Partial<VehicleFormData> = {
                id_marca: values.brand,
                id_modelo: values.model,
            ...
            const errorMessage = err instanceof Error ? err.message : 'No se pudo actualizar el vehículo';
